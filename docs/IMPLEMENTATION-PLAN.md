...
| ↳ *P1 status (2026-08-15)* | **Landed, QA-reviewed** (352 tests). Mutation-intent envelope (ADR-007); Task/WBS CRUD + in-process rollup scheduler and calendar CRUD (`apps/api`); Task/WBS tree and calendar UI (`apps/web`). QA found and fixed two real bugs (cross-tenant calendar reference/existence oracle; unvalidated half-day exception range) and deepened RBAC/id-oracle/audit coverage. **Carried forward, not silently dropped:** FR-TRK-04's parent-level manual override (needs a schema column P0 lacks), FR-CAL-04's regional holiday data (product decision, pinned as a failing test), the rollup's per-row query pattern (needs a set-based redesign to hold FR-SCH-06's budget — P2's problem, noted in the risk register), no axe/e2e automation yet. | — | ADR-007 resolved the queue question: no ADR-002 queue needed until P3 |
...
| Performance targets (500ms/5k tasks) don't hold once real calendars/constraints are layered on top of the naive algorithm | Medium | Medium | Budget the dedicated P8 hardening phase; add perf regression tests to CI (not just manual spot-checks) as soon as P2 lands. **Confirmed materializing early:** P1's QA review found the rollup's per-row query pattern (one round trip per ancestor, one per renumbered sibling) already won't hold the budget at CPM scale — `scheduler-engineer` needs a set-based recompute design in P2, not a patch on the P1 code |
...
| No automated accessibility or cross-browser regression coverage exists yet (axe/jest-axe, Playwright e2e) | Medium | Medium | `docs/MODEL-ROUTING.md`/`qa-engineer`'s charter call for this starting P1, not P8; P1 shipped component-level accessibility tests but no axe integration and no e2e harness at all — add both before P2's UI surface (dependency editing, critical-path rendering) makes the gap more expensive to close |
